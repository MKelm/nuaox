...
NX.Pixi.prototype.constructor = NX.Pixi;

NX.Pixi.prototype.autoResizeRenderer = function() {
...
NX.Pixi.prototype.animate = function(scope) {
  ...
  if (nx.game.run == true) {
    nx.game.update(nx.game);
  }
  scope.renderer.render(scope.stage);
  TWEEN.update();

  if (nx.userConfig.fps > -1) {
    global.setTimeout(function() { requestAnimFrame(scope.animate.curry(scope)); }, 1000 / nx.userConfig.fps );
  } else {
    requestAnimFrame(scope.animate.curry(scope));
  }
}

NX.Pixi.prototype.setLoader = function(activate) {
  if (activate === true) {
...
NX.Pixi.prototype.loadAssets = function(callback) {
  this.setLoader(true);
  var assetsToLoad = [
    "data/gfx/blank.png", "data/gfx/ships/player.png",
    "data/gfx/ships/fox.png", "data/gfx/ships/viper.png",
    "data/gfx/bullets/gun.png", "data/gfx/bullets/minigun.png",
    "data/gfx/items/pointsup.png", "data/gfx/items/shieldup.png"
  ];
...
NX.Pixi.prototype.setFpsCounter = function(keyEvent, scope) {
  ...
  if (typeof scope.fps == "undefined") {
    ...
    if (keyEvent === true && scope.fps[0] < 0) {
      ...
      nx.intervals["fpsCounter"] = setInterval(scope.setFpsCounter.curry(false, scope), 1000);
      keyEvent = false;
    ...
    if (keyEvent === true && scope.fps[0] > -1) {
      clearInterval(nx.intervals["fpsCounter"]);
      scope.fps[1].visible = false;
      scope.fps[0] = -1;
