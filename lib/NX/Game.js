...
NX.Game = function() {
  ...
  this.itemHandler = new NX.ItemHandler(this);
  ...
  this.addEventListener('player-item-collision', nx.util.getEventListener(this, "handleEvent"));
  ...
  if (ship) {
    if (ship.setBulletHit(damage) == false) {
      ...
      if (ship.handle !== "player") {
        // chance to drop item
        if (nx.util.isChance(0.5)) {
          var item = this.itemHandler.getRandomItem();
          item.position = this.display.getShipPosition(shipDisplayId);
          this.itemHandler.displayItem(item);
        }
      }
      this.display.removeShip(shipDisplayId);
      if (ship.handle == "player" || this.shipHandler.isRushsEnd()) {
        // stop game and show game over window
        this.run = false;
        this.display.stopShips();
        var scope = this;
        if (this.points > 0) {
...
NX.Game.prototype.handleEvent = function(scope, event) {
  switch (event.type) {
    case "window-close-click":
      ...
      break;
    case "player-item-collision":
      scope.itemHandler.applyItemEffects(event.content.item.id);
      break;
