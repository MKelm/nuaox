...
NX.Game.prototype.start = function() {
  ...
  this.ships[0].id = this.display.drawShip("player");
...
NX.Game.prototype.update = function(scope) {
  ...
  for (var key in nx.keys) {
    if (key == 37) { // left arrow
      scope.display.moveShip(scope.ships[0].id, -1 * scope.ships[0].speed / scope.fps, 0);
    } else if (key == 39) { // right arrow
      scope.display.moveShip(scope.ships[0].id, scope.ships[0].speed / scope.fps, 0);
    } else if (key == 38) { // up arrow
      scope.display.moveShip(scope.ships[0].id, 0, -1 * scope.ships[0].speed / scope.fps);
    } else if (key == 40) { // down arrow
      scope.display.moveShip(scope.ships[0].id, 0, scope.ships[0].speed / scope.fps);
    }
  }
