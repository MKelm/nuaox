...
NX.Game.prototype.update = function(scope) {
  ...
  for (var key in nx.keys) {
    if (key == 37 || key == 39 || key == 38 || key == 40) {
      ...
      if (scope.display.areEnemyShipsInPlayer(scope.shipHandler.player, scope.shipHandler.enemies)) {
  ...
  for (var i = 0; i < scope.shipHandler.enemies.length; i++) {
    ...
    // todo: add bullet handling for enemies
  }
...
NX.Game.prototype.handleEvent = function(scope, event) {
  switch (event.type) {
    ...
    case "bullet-ship-collision":
      var ship = scope.shipHandler.getEnemyShipByDisplayId(event.content.shipDisplayId);
      if (ship) {
        if (ship.setBulletHit(event.content.damage) == false) {
          scope.display.removeShip(event.content.shipDisplayId);
          scope.shipHandler.removeShip(
            scope.shipHandler.getEnemyShipByDisplayId(event.content.shipDisplayId, true)
          );
        }
      }
      break;
