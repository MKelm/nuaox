...
NX.Game.prototype.update = function(scope) {
  ...
  for (var key in nx.keys) {
    if (key == 37) { // left arrow
      scope.display.moveShip(-1 * scope.player.speed / scope.fps, 0);
    } else if (key == 39) { // right arrow
      scope.display.moveShip(scope.player.speed / scope.fps, 0);
    } else if (key == 38) { // up arrow
      scope.display.moveShip(0, -1 * scope.player.speed / scope.fps);
    } else if (key == 40) { // down arrow
      scope.display.moveShip(0, scope.player.speed / scope.fps);
    }
  }
