...
NX.ShipHandler.prototype.initializeRush = function() {
  ...
  for (var i = 0; i < this.enemies.length; i++) {
    ...
    enemy.nextStep = 1;
    enemy.animate = false;
    enemy.lastShootCheck = { time: 0, chance: false };
  }
}

// detect if an enemy pilot wants to shoot
NX.ShipHandler.prototype.isEnemyShipShootActive = function(enemyId) {
  var time = nx.util.time();
  if (this.enemies[enemyId].lastShootCheck.time < time - 1000) {
    this.enemies[enemyId].lastShootCheck.time = time;
    this.enemies[enemyId].lastShootCheck.chance = nx.util.isChance(0.3, 10);
  }
  return this.enemies[enemyId].lastShootCheck.chance;
}
  ...
  for (var i = 0; i < this.enemies.length; i++) {
    if (this.enemies[i].animate == false) {
      if (this.enemies[i].nextStep == this.enemies[i].steps.length) {
        this.display.removeShip(this.enemies[i].ship.displayId);
...
NX.ShipHandler.prototype.removeShip = function(id) {
  this.enemies.splice(id, 1);
}

NX.ShipHandler.prototype.getShipByDisplayId = function(displayId, getId) {
  if (displayId == this.player.displayId) return this.player;
  for (var i = 0; i < this.enemies.length; i++) {
    if (this.enemies[i].ship.displayId == displayId) {
      return (getId) ? i : this.enemies[i].ship;
    }
  }
  return false;
}