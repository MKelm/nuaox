...
NX.Display.prototype.drawShip = function(handle) {
  var sprite = new PIXI.Sprite(PIXI.Texture.fromImage("data/gfx/ships/"+handle+".png"));
  sprite.position = { x: -50, y: (nx.pixi.screen.height/2) - 100 };
  this.container.addChild(sprite);
  this.ships.push(sprite);
  return this.ships.length - 1;
}

NX.Display.prototype.moveShip = function(id, addX, addY) {
  if (addX !== 0) {
    this.ships[id].position.x += addX;
  }
  if (addY !== 0) {
    this.ships[id].position.y += addY;
  }
}
