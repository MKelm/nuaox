...
NX.Display.prototype.animateEnemyShip = function(enemy) {
  ...
  if (typeof ship != "undefined" && typeof targetPosition != "undefined") {
    ...
    enemy.animate = true;
    var tween = new TWEEN.Tween( { x: ship.position.x, y: ship.position.y } )
      .to(
      ...
      .onUpdate( function () {
        ...
        if (typeof scope.ships[0] != "undefined") {
          var enemyPlayerDistance = Math.sqrt(
            Math.pow(ship.position.x - scope.ships[0].position.x, 2) +
            Math.pow(ship.position.y - scope.ships[0].position.y, 2)
          );
          // needs ships with width==height!
          if (enemyPlayerDistance < ship.width + scope.ships[0].width) {
