...
NX.Display.prototype.initialize = function() {
  ...
  this.starsGfx.position = {x: -1 * nx.pixi.screen.width/2, y: -1 * nx.pixi.screen.height/2 };
    ...
    if (ship.weapons[i].lastAction == -1 || lastActionTimeDiff > ship.weapons[i].delay) {
      ...
      tweens[i] = function (iI, iSprite, iTargetPosition, iDistance, iShip, iWeaponId) {
        return new TWEEN.Tween( { x: iSprite.position.x, y: iSprite.position.y } )
          ...
          .onComplete( function() {
            if (typeof scope.container.children.indexOf(iSprite) == "number") {
              scope.container.removeChild(iSprite);
            }
          })
          .start();
...
NX.Display.prototype.drawStars = function(init) {
  if (this.stars.length < this.maxStars) {
    for (var i = this.stars.length; i < this.maxStars; i++) {
      this.stars[i] = {
        ...
        x: Math.random() * nx.pixi.screen.width,
        y: (init) ? Math.random() * nx.pixi.screen.height : 0
      };
    }
  }
  this.starsGfx.clear();
  ...
  for (var i = 0; i < this.stars.length; i++) {
    if (this.stars[i].y > nx.pixi.screen.height) {
      starsToRemove.push(i);
    } else {
      if (!init) this.stars[i].y += this.stars[i].speed / this.game.fps
