...
NX.Display.prototype.getPlayerShipInTargetRange = function(player, enemy) {
  ...
  if (typeof enemyShip != "undefined") {
    ...
    return nx.util.isRectangesCollision(
      ...
      playerShip.position.x, playerShip.position.y, player.size, player.size
    );
  }
  return false;
}

NX.Display.prototype.getEnemyShipsInPlayer = function(player, enemies) {
  var collision = null;
  var playerShip = this.ships[player.displayId];
  for (var i = 0; i < enemies.length; i++) {
    var enemyShip = this.ships[enemies[i].ship.displayId];
    // detects if enemies has a collision with player
    var distance = Math.sqrt(
      Math.pow(enemyShip.position.x - playerShip.position.x, 2) +
      Math.pow(enemyShip.position.y - playerShip.position.y, 2)
    );
    if (distance < player.size/2 + enemies[i].ship.size/2) {
      if (collision === null) {
        collision = [enemies[i].ship.displayId];
      } else {
        collision.push(enemies[i].ship.displayId);
      }
    }
  }
  return collision;
...
NX.Display.prototype.drawPoints = function(points) {
  ...
  var tPoints = new PIXI.Text(prefix+points, style);
...
NX.Display.prototype.drawShield = function(shield) {
  var prefix = "Shield: ";
  var style = {font: 26 + "px " + "Arial", fill: "FFFFFF"};
  var tShield = new PIXI.Text(prefix+shield, style);
  if (this.shieldText !== null) {
    this.container.removeChild(this.shieldText);
  }
  this.container.addChild(tShield);
  this.shieldText = tShield;
  this.shieldText.position = { x: -400+20, y: 300-this.shieldText.height-20 };
}
