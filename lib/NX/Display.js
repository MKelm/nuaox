...
NX.Display.prototype.animateEnemyShip = function(enemy) {
  ...
  var dspShip = this.ships[enemy.ship.displayId], targetPosition = enemy.steps[enemy.nextStep];

  var distance = Math.sqrt(
    Math.pow(dspShip.position.x - targetPosition.x, 2) + Math.pow(dspShip.position.y - targetPosition.y, 2)
  );

  enemy.animate = true;
  new TWEEN.Tween( { x: dspShip.position.x, y: dspShip.position.y } )
    .to(
      { x: targetPosition.x, y: targetPosition.y }, 1000 * (distance / enemy.ship.speed)
    )
    .onUpdate( function () {
      dspShip.position.x = this.x;
      dspShip.position.y = this.y;
    })
    .onComplete( function () {
      enemy.animate = false;
      enemy.nextStep++;
