...
NX.Display.prototype.drawShip = function() {
  var sprite = new PIXI.Sprite(PIXI.Texture.fromImage("data/gfx/ship.png"));
  sprite.position = { x: -50, y: (nx.pixi.screen.height/2) - 100 };
  this.container.addChild(sprite);
  this.ship = sprite;
}

NX.Display.prototype.moveShip = function(addX, addY) {
  if (addX !== 0) {
    this.ship.position.x += addX;
  }
  if (addY !== 0) {
    this.ship.position.y += addY;
  }
}
