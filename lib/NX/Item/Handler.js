...
NX.ItemHandler = function(game) {
  ...
  this.items = [
    ...
    { handle: "extrapoints", effects: { points: 100 } }
  ]
}
...
NX.ItemHandler.prototype.getRandomItem = function() {
  var id = Math.round(Math.random() * this.items.length);
  return { handle: this.items[id].handle, id: id };
}


NX.ItemHandler.prototype.applyItemEffects = function(item) {
  var effects = this.items[item.id].effects;
  for (var property in effects) {
    switch (property) {
      case "shield":
        game.shipHandler.player.shield += effects.shield;
        break;
      case "points":
        game.points += effects.points;
        break;
    }
  }
}
...
NX.ItemHandler.prototype.displayItem = function(item) {
  if (typeof item.position == "object") {
    this.game.display.drawItem(item.handle, item.position);
  }
}